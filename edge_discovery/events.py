...
def event_range_compression_break(df: pd.DataFrame, lookback: int = 64, q_low: float = 0.2, k_atr: float = 1.4, confirm_bars: int = 20) -> pd.Series:
...
def event_impulse_pullback(df: pd.DataFrame, k_atr: float = 1.2, pullback_min: float = 0.25, pullback_max: float = 0.6, within_bars: int = 20) -> pd.Series:
...
def event_liquidity_void_fill_proxy(df: pd.DataFrame, gap_k_atr: float = 1.6, fill_bars: int = 20, ema_span: int = 20) -> pd.Series:
...
def build_event_matrix(df: pd.DataFrame, config: dict | None = None) -> pd.DataFrame:
    ...
    out["E_range_compression_break"] = event_range_compression_break(
        ...
        confirm_bars=int(cfg.get("compression_confirm", 20)),
    )
